        ...
        try:
            ...
            for chapter in report:
                if chapter.get('id') == 'sub-2-1':
                    chapter['content'] = chapter['content'].replace('width:15%;', 'width:30%;').replace('width:65%;', 'width:50%;')
                    modified = True
                
                if 'subsections' in chapter:
                    for sub in chapter['subsections']:
                        if sub.get('id') == 'sub-2-1':
                            sub['content'] = sub['content'].replace('width:15%;', 'width:30%;').replace('width:65%;', 'width:50%;')
                            modified = True
