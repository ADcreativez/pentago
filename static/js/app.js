let activeTab = window.location.pathname === '/' ? 'dashboard' : window.location.pathname.substring(1);
    ...
    fetch('/api/auth/me')
        ...
        .then(data => {
            if (data.logged_in) {
                ...
                // If we are on a workspace endpoint, open the project automatically
...
function switchTab(tab) {
    ...
    // Check if the target view exists in the current DOM
    // If not, we are on a different HTML page (like workspace.html) and need to reload
    let targetView = document.getElementById(tab + '-view');
    
    // Special case for dashboard which might be its own page or in index.html
    if (!targetView) {
        // Prevent infinite loop by checking if we are already on the target path
    ...
    // Update URL to match endpoint without reloading the page
    if (tab === 'dashboard') {
...
function renderMarkdownToHtml(txt, imgCounterObj = { val: 0 }) {
    ...
    if (typeof marked !== 'undefined') {
        ...
        if (typeof DOMPurify !== 'undefined') {
            parsedHtml = DOMPurify.sanitize(parsedHtml, { ADD_ATTR: ['class', 'style'] });
        }
    } else {
...
async function openReportPreview(lang = 'id') {
    ...
    try {
        ...
        try {
            ...
            // Build with selected language to allow hardcoded structural translations
            previewHtml = _buildPreviewDocument(p, findings, null, structure, lang, false, spacingMode);
            
            // If English is requested, translate the remaining content using the backend API
            if (lang === 'en') {
                try {
...
window.initWorkspaceEditors = function() {
    ...
    const toolbarOptions = {
        container: [
            ...
            ['link', 'image', 'video', 'formula', 'table', 'caption', 'row-add', 'row-del', 'col-add', 'col-del', 'tick-green', 'tick-red'],
        ...
        handlers: {
            caption: function() {
                ...
                if (range) {
                    this.quill.insertText(range.index, '\n[caption: Judul Gambar Anda di Sini]\n');
                    this.quill.setSelection(range.index + 11, 25);
                }
            },
            'tick-green': function() {
                const range = this.quill.getSelection(true);
                if (range) {
                    this.quill.clipboard.dangerouslyPasteHTML(range.index, '<span style="color:#16a34a; font-weight:bold; font-size:1.1rem;">&#10003;</span>');
            ...
            'tick-red': function() {
                const range = this.quill.getSelection(true);
                if (range) {
                    this.quill.clipboard.dangerouslyPasteHTML(range.index, '<span style="color:#dc2626; font-weight:bold; font-size:1.1rem;">&#10003;</span>');
                    this.quill.setSelection(range.index + 1);
                }
            },
            'row-add': function() {
            ...
            table: function() {
                const rowsStr = prompt("Masukkan jumlah baris (contoh: 3):", "3");
                if (rowsStr === null) return;
                const colsStr = prompt("Masukkan jumlah kolom (contoh: 3):", "3");
                ...
                if (rows > 0 && cols > 0) {
                    ...
                    alert("Jumlah baris dan kolom harus berupa angka lebih dari 0.");
                }
            }
        }
    };
...
window.renderWorkspaceChapters = function() {
    ...
    window.currentTechReport.forEach((sec, sIdx) => {
        const hasTable = sec.content && sec.content.includes('<table class="tbl"');
        html += `
