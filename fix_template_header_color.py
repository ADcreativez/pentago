        ...
        try:
            ...
            for chapter in report:
                if chapter.get('id') == 'sub-2-1':
                    chapter['content'] = chapter['content'].replace("background-color:#e50000", "background-color:#1e3a5f")
                    modified = True
                
                if 'subsections' in chapter:
                    for sub in chapter['subsections']:
                        if sub.get('id') == 'sub-2-1':
                            sub['content'] = sub['content'].replace("background-color:#e50000", "background-color:#1e3a5f")
                            modified = True
