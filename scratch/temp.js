...
function _buildPreviewDocument(p, findings, tpl, structure, lang = 'id', isDocx = false, spacingMult = 1.4) {
    ...
    const estimateHtmlHeight = (html) => {
        ...
        let h = Math.max(30, Math.ceil(cleanText.length / 55) * 24 * spacingMult);

        const h2Count = (html.match(/<h2/g) || []).length;
        const h3Count = (html.match(/<h3/g) || []).length;
        h += (h2Count * 60 + h3Count * 45) * spacingMult;

        // Increase tr base height to 50, and DO NOT multiply by spacingMult 
    ...
    if (workspaceDocs['sub-1-6'] !== undefined) {
        const owaspChecklistHtml = `<h3 class="ssh">${tr("1.6. OWASP TOP 10 Checklist")}</h3>${renderContent(workspaceDocs['sub-1-6'])}`;
        flowItems.push({
            type: 'general',
        ...
        const theadHtml = `<thead><tr>${owaspColumns.map(col => {
            let style = "";
            if (col.type === 'pass' || col.type === 'issue') style = ' style="text-align:center;"';
    ...
    flowItems.forEach(item => {
        // Use 910 as threshold to pack content securely without overlapping the footer
        if (currentChunkHeight + item.height > 910 && currentChunk.length > 0) {
