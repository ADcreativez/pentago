...
c.execute("SELECT id, technical_report, used_tools FROM project")
rows = c.fetchall()

for row in rows:
    p_id, report_str, used_tools = row
            ...
            if 'subsections' in ch:
                for sub in ch['subsections']:
                    ...
                    if sub.get('id') == 'sub-2-2' or 'Penetration Testing Tools' in sub.get('title', ''):
                        sub['content'] = get_new_2_2(used_tools or "Maltego, Dnsenum, Theharvester, Nmap, Nessus Pro, Nikto, w3af, Acunetix Pro, Zaproxy, Sqlmap, Metasploit, Burpsuite Pro, exploit-db, Dirb")
                        modified = True
        if modified:
            c.execute("UPDATE project SET technical_report = ? WHERE id = ?", (json.dumps(data), p_id))
    except Exception as e:
        print(f"Error on id {p_id}: {e}")

c.execute("SELECT id, structure FROM report_template")
rows = c.fetchall()

for row in rows:
    t_id, struct_str = row
    if not struct_str: continue
    try:
        data = json.loads(struct_str)
        modified = False
        for ch in data:
            if 'subsections' in ch:
                for sub in ch['subsections']:
                    ...
                    if sub.get('id') == 'sub-2-2' or 'Penetration Testing Tools' in sub.get('title', ''):
                        ...
                        modified = True
        if modified:
            c.execute("UPDATE report_template SET structure = ? WHERE id = ?", (json.dumps(data), t_id))
    except Exception as e:
        pass
